describe('Buttons', ()=>{
    ...
    it('Cenário Negativo: Não deve exibir mensagem com apenas 1 clique', () => {
        cy.get('#doubleClickBtn').click();
        cy.get('#doubleClickMessage').should('not.exist');
    });


    it('Cenário Negativo: Não deve exibir mensagem com clique simples', () => {
    ...
    it('Falha ao tentar clicar em botão inexistente', () => {
        cy.get('button').filter((index, el) => el.innerText.trim() === 'Click Me')
            .should('exist')
            .and('be.visible');
});
